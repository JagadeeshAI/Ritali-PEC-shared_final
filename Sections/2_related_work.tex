...
On the memory side, 3D-Mem~\cite{huang20243dmem} builds volumetric spatial memories and MemoryExplorer~\cite{wang2026memoryexplorer} structures episodic memory with spatial-temporal annotations. MemoryExplorer introduces the retrieval-augmented baseline RA-Mem, a variant of 3D-Mem, together with the LMEE-Benchmark dataset~\cite{wang2026memoryexplorer}, which couples multi-goal navigation with memory-based question answering, naming several objects to reach in sequence and afterwards querying the agent about what it observed.
...
These methods enrich what the agent remembers, but the search deciding what it sees remains governed by a static scorer. As Table~\ref{tab:qualitative} summarises, no prior method adapts its scorer within an episode, which is what sets PEC-Nav apart.