...
\paragraph{Explore.} The agent selects $f^* = \arg\max_f \mathrm{val}'(f)$ (Eq.~\ref{eq:calibrate}--\ref{eq:visitpenalty}) and navigates toward it with a point-goal controller, recording egocentric RGB observations into an episodic memory buffer with spatial-temporal coordinates.
