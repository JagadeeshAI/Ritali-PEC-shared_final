...
\begin{equation}
    ...
    \label{eq:arrival}
\end{equation}
where $w_f$ is the frontier's stored world-frame position and $\mathrm{pts}_{\text{norm}}$ the agent's position after that transform, and correction fires once Eq.~\ref{eq:arrival} falls below $R$. It then builds an observed distribution over the same support,
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:kl}
\end{equation}
The divergence in Eq.~\ref{eq:kl} is bounded, since flooring both arguments at $\epsilon$ caps any single term at $\log(1/\epsilon) \approx 6.9$ nats. A confident prediction that is fully contradicted therefore produces a large but finite surprise, keeping the signal at an $O(1)$ magnitude.

\subsection{Self-Calibration}
\label{sec:calibration}

Surprise drives two feedback paths through a shared running estimate. We maintain an exponential moving average (EMA) of per-correction values and squash it to a bounded signal,
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:adapt}
\end{equation}
The regime Eq.~\ref{eq:adapt} produces matters for reading our results. The map is strongly compressive above $\bar{s} \approx 2$, where every value sends $n_t$ into $[0.86, 1)$ and $\sigma_t$ into $(0.4, 0.48]$, so it discriminates sharply among small surprises and barely among large ones, and with $\beta = 0.3$ the EMA of Eq.~\ref{eq:ema} reaches this regime within roughly five corrections. The intent is fast commitment rather than gradual annealing, with the floor bounding how far distrust can go.
...
At each step the agent scores every open frontier, stores a prediction for it, moves to the highest-valued one, and on reaching a previously predicted frontier measures the surprise and updates $\sigma_t$ and the weights, as Algorithm~\ref{alg:pecnav} sets out.

\subsection{Memory, QA and Implementation}
\label{sec:memory}

Every egocentric RGB observation is stored with spatial-temporal coordinates, and follow-up questions are answered by retrieving and reasoning over them. Across our ablations memory and QA are held identical with only search varying, so changes in answer quality there are attributable to search. This does not extend to the published comparison systems, whose memory modules differ.

\begin{table}[htbp]
\centering
\caption{PEC-Nav hyperparameters.}
...
Question answering runs locally on a Qwen2.5-VL-7B~\cite{bai2025qwen25vl} model, with the full protocol in Sec.~\ref{sec:setup}. Table~\ref{tab:hyperparams} lists all hyperparameters, tuned on 5 development scenes and then fixed.
