...
\caption{PEC-Nav explores by frontier search that adapts online from its own prediction errors, collects episodic memories, and answers follow-up questions from stored observations.}
\label{fig:teaser}
\end{figure*}
...
We instantiate this in Predict-Explore-Correct Navigation (PEC-Nav), an EQA pipeline whose search module forms a closed feedback loop. An LLM parses the instruction into a target and its relational context, a predictor anticipates the layout beyond each frontier, an uncertainty-aware policy balances goal relevance against prediction confidence, and a correction module updates the predictor's confidence scaling and the explore-exploit weights online. Observations along the way populate an episodic memory that answers follow-up questions, and because memory and QA are fixed across our ablations, changes in answer quality there are attributable to search.

On LMEE-Bench's official 58-task subset~\cite{wang2026memoryexplorer}, using GPT-4o for goal extraction and search reasoning and Qwen2.5-VL-7B for memory-based question answering, PEC-Nav reaches 24.45\% SR, 16.86 SPL, 57.93 MLLM-Score and 66.21\% QA accuracy.
